...
To better understand effects of rest and home/away advantage the future research should focus on distances the teams have to travel between the games. For example if a team is travelling from one coast to another- it is likely to affect them more comparing to a team from New York 'travelling' to play an away gamein New Jersey.

<<<<<<< HEAD
##Effects on accuracy
=======
>>>>>>> 30a99b72c77aee586827ca60570eb176b1e55b10
...
<<<<<<< HEAD
=======
The aim of this analysis is to examine whether the top scorer of a team can help his team to win the match when he has a more outstanding performance in terms of both number of shooting attempts and accuracy. The hypothesis is that a team will be more likely to win the game if the top scorer have more opportunities to shot in a single match as this player tend to score than others players in the team. Furthermore, a team is more likely to win when the key player has higher shot percentage than normal. 
...
2. A new subset  was created so that each row contained the player’s name, match details and shooting performance of that player.
...
5. Plot results. 
6. Investigate the correlation between the results of the matches and the performances of top scorers. 
7. A further logistic regression test was run in order to examine three special players: Russell Westbrook, Kawhi Leonard and Gordon Hayward. 
>>>>>>> fe0134f116f5f5d78d1c886aa59bc28fb130f542
...
<<<<<<< HEAD
=======
There was no correlation found between the result of a match and the number of attempts of the player made in that match. The Pearson’s correlation coefficient for the relationship between the variables W and FGA was r(832) < .01, p = .794, failing to reach level of significance (p = .05). 

##b.  Field Goal Percentage (FG%)

There was no correlation found between the result of a match and the field goal percentage of the player in that match. The Pearson’s correlation coefficient for the relationship between the variables W and FGA was r(832) < .01, p = .177, failing to reach level of significance (p = .05). 
...
```
Table xxx: The logistic regression for FGA and FG% of 15 players.
...
Table xxx: The logistic regression for FGA and FG% of the 3 special players.
| | Estimate | Std. Error | t value | Pr(>t)   
|-|-|-|-| - 
|(Intercept)   | 0.042259 |  0.164816 |  0.256  |  0.798    
|FGA          | -0.002679 |  0.006468|  -0.414  |    0.679    
...
The box plots above shows the performances the three special players in terms of FGA and FG% respectively. The red (blue) boxes contain the data when the team lost (won) the match. 

###Discussion

From the correlation tests between the match results and players’ performances, it can be noticed that the increase in number of field goal attempts from top scorers is unlikely to make any impact on the results of the game. The correlation coefficient between the results of the game and FG% is sightly higher but these two factors are still not correlated since r(832) < .01. Meanwhile, the p-values of both correlation tests are too higher to reach the default level of significance (p = .05).

From the logistic regression test for FGA and FG%. It was found that FGA did not have any impact on the result of the match. FG% had significant correlation with the result of match. However, the anova test showed that the deviances of these two factors were both quite large and thus further investigations must be done in order to improve this model.

There was no correlation found between field goal attempts of top scorers and game results. A possible explanation is that it is easier for a team to focus on a certain player on defensive side. Thus, over-using the scoring ability of a leading player is not likely to be a good approach if the team wants to win the game. Meanwhile, the accuracy of these players has a stronger impact on the outcome of the game. This is due to fact that once a player is being efficient on the offensive side, that player can help the team by not only keeping shooting but also creating more opportunities for his teammates. 

Based on results above, further research has been done regarding to the performances of three special players: Russell Westbrook, Kawhi Leonard and Gordon Hayward. These three players were chosen because their teams are more likely to take the victory when they score more in single game according to the bar plot and t-test. 

For these three players, the p-values still did not reach the level of significance (p = .05) for FGA. However, all p-values are smaller than 0.05 when their FG%s were tested. Based on these findings, another logistic regression was done with these three players. It was shown that p < .001 and residual deviance from anova dropped by 4.498 which is about 12.3% when FG% was introduced. In comparison, the deviance was above 200 when 15 players were tested. 

Therefore, these three players are somehow different from others as they can contribute more to the win rates of their teams by their increasing shooting accuracy. There could be many reasons behind this, including their team compositions and the roles of the players.

#Limitations

The data set only contains 904 out of 1230 matches in the entire regular season. So there might be a chance that the highlight performances of certain players are missed out in our analysis. Meanwhile, it would be ideal if the contributions of all players can be analysed in more aspects including number of rebounds and assists. 


##Defender Proximity Analysis  
###Closest Defender Distance vs Shot Accuracy
####Introduction  
This analysis focuses on whether there is a significant effect from closest defender distance to the shot outcome. The Null Hypothesis is that the distance to a closest defender does not have an impact on  further a defender from the shooter, then it will be more likely to increase the chance of scoring. This is because the shooter is practically in "free" condition without any physical interference.  
...
##Hot Hand Theory
###Introduction
  The analysis aims to show the effect of the 'hot hand theory' on NBA players. The analysis is based on the hypothesis that a shooter with a hot hand will have shots that are dependent of one another. If the shooter makes his first shot, the probability of making his next shot is higher.
...
#Createing a function that gives a vector with the number of total streaks(from all players and all games)
...
####Plot Result
  After running many times the sample shooter, there are times that the distributions of sample and real shooter are quite similar. It can be concluded that Mo Williams does not have a hot hand.
...
####Plot Result
  ...
  Klay Thompson is an exception making 13 shots in a row. There is no such distribution in the sample, so it could be assumed that he is likely to have a hot hand. Further statistical analysis - logistic regression - should be done in order to reach a conclusion.

###Discussion

  As the analysis shown, there are some extremely high shot streaks achieved from the players. Kyrie Irnving achieved six shots in a row, Mo Williams 4 shots in a row twice, while Klay Thompson 13 shots in a row. However, to assume that a player has a hot hand, it is required to prove that his continuous shots are dependent one of another. As the vectors of streaks do not contain continuous variables, a linear regression cannot be used. Creating a sample independent shooter with same number of attempts and same shooting percentage as the real's one, is the closest approach to the specific question. In the first two cases, there are several times when the distribution of streaks of the sample independent player is similar to the real shooter's distribution. As the sample shooter's distribution visualises independent shots, it can be concluded that Kyrie Irnving's and Mo Williams' shots are independent one of another and as a result they have not a hot hand. On the other hand, Klay Thompson is likely to have a hot hand, as his shots length distribution diverges significantly from the sample's distribution.   
 
###Limitations

  The dataset is referred to only one season
