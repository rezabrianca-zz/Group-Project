...
>>>>>>> e886b4bc6d1f0f9e0cddd3b631ecbf124c44b7bc
a. Hot Hand Theory (Artemis)
b. Defender Proximity Analysis (Reza)
c. Shot Clock Pressure  (Alex)
d. Close Game Effect - Small Margin affecting the scoring rate 
...
gg1 <- ggplot(data = player1_df, aes(x = streaks)) + geom_histogram(binwidth = 1) + labs(title = ourtable[1,'player_name'])
gg2 <- ggplot(data = player2_df, aes(x = streaks)) + geom_histogram(binwidth = 1) + labs(title = ourtable[2,'player_name'])
gg3 <- ggplot(data = player3_df, aes(x = streaks)) + geom_histogram(binwidth = 1) + labs(title = ourtable[3,'player_name'])
...
grid.arrange(g1, g2, ncol = 2)
#Klay Thompson is an exception, had 13 streaks in a row! We cannot find a similar distribution in our sample, so we can assume that he is likely to have a hot hand.
