...
write.csv(df,"shot_logs1010.csv")
```
...
=======
aa2<- cast(aa, AWAY_TEAM ~ WINNER); names(aa2)[1]<-"TEAM"
>>>>>>> 961cfa1e3730130a219fb77b1874aa2800a0e664
...
<<<<<<< HEAD
#aa3$WIN_PCT_HOME <- with(aa3, reorder(WIN_PCT_HOME, WIN_PCT_HOME, function(aa3) - length(aa3)))
#ggplot(aa3) + geom_point(aes(WIN_PCT_HOME))

aa3<-aa3[order(aa3$WIN_PCT_HOME,decreasing=TRUE),]
head(aa3)

#home_pct <- table(aa3$WIN_PCT_HOME)
#home_pct_level <- names(home_pct)[order(home_pct)]
#aa3$WIN_PCT_HOME2 <- factor(aa3$WIN_PCT_HOME, levels = home_pct_level)
head(aa3)

ggplot(aa3,aes(x=TEAM,y= WIN_PCT_HOME,colour = "HOME")) +geom_point() + geom_point(aes(y= WIN_PCT_AWAY, colour ="AWAY"))


#ggplot(aa3,aes(x=TEAM))+ geom_point(aes(y= WIN_PCT_HOME,colour = "HOME")) + geom_point(aes(y= WIN_PCT_AWAY, colour ="AWAY"))
#TO BE UPDATED
=======
aa3<- aa3[order(aa3$WIN_PCT_HOME,decreasing=TRUE),]
rownames(aa4)<-NULL
...
for (z in 1:nrow(df)){
  if(df[z, "SHOT_RESULT"] == 1 & df[z, "PTS_TYPE"] >= 1){
    ...
    shot_dist <- c(shot_dist, df[z, "SHOT_DIST"])
  }
}

#combine point type and distance
valid_distance = data.frame(point_type, distance, shot_dist)
dist_group <- c(3, 6, 9, 12, 15, Inf)
valid_distance[, "Range_Group"] <- c(findInterval(valid_distance$distance, dist_group))

#calculate correlation between defender distance and points made (2 or 3 points)
cor(point_type, distance)

#calculate correlation between defender distance and shot distance (2 or 3 points)
cor(distance, shot_dist)
...
ggplot(data = valid_distance, aes(x = Range_Group)) + geom_bar()

ggplot(data = valid_distance, aes(x = distance) + geom_bar())
...
#calculate mean distance from 2-pointers
avg_dist2 <- aggregate(distance2 ~ players2, player2point, mean)
