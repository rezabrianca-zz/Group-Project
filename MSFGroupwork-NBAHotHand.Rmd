...
#creating a column for 'TEAM' regardless whether they played at home or not as the original dataframe consists of both home and away teams
for (i in 1:nrow(df1)) df1$TEAM[i] <-unlist(strsplit(df1$MATCHUP[i], " "))[5]


#cleaning-up data for rest days calculation
df1$BLANK2<-""
...
#regression analysis
restreg<- lm(FINAL_MARGIN ~ LOCATION + rest, restdays2)
summary(restreg)
...
Literature review demonstrated that a team playing at home is more likely to win with an average probability of the home win is around 60 percent. (http://www.economist.com/blogs/gametheory/2015/06/home-advantage-basketball) As a result, part of this research is aimed at analysing whether home team advantage was statistically evident in season 2015-2016. 

The NULL Hypothesis is: Home playing at home is more likely to win at home then when playing away.
...
for (i in 1:nrow(aa3)) {
  if(aa3$WIN_PCT_HOME[i] > aa3$WIN_PCT_AWAY[i]) aa3$HOME_ADV[i] <- "YES" 
  else aa3$HOME_ADV[i] <- "NO"
}
head(aa3)
...
The plot shows that out of 30 teams:
  ...
  2 teams home and away win ratios are almost equal (difference of less than XXX)
...
The analysis shown that there is a clear evidence of a team playing at home is more likely to win with an average win probability of 55.97%. The same conclusion is evident from the analysis on a team level where 25 teams are clearly winning more games at home and away. The result confirm acceptance of the NULL hypothesis, which stated that a team playing at home is more likely to win then an away team.

The result of this research also confirms recent trend of decline in home advantage over the years: probability of a home team winning has reduced from around 65% in 1975-1992 to average of 60.3% in 1993-2011 and to a figure around the 2011-15 mean of 58.8%. Home win ratio was 57.4% win in 2014-2015 season. The analysis demonstrated that the home win ratio went down even further to 56.0% in 2015-2016 season. Based on the literature review this trend could be potentially explained by the following factors http://www.economist.com/blogs/gametheory/2015/06/home-advantage-basketball:

  1. Improvement of travel conditions. This means that when playing away, teams are now more likely to stay at better hotels and fly charterd planes more often than in the past. This results in teams being less tired as a result of travel and better prepared for the games.
  
  2. Change in style of play in the NBA. The game now is more open with less physical contact, which reduces number of fouls and potential impact of 'notorious home-team bias' by referees.
...
The steps to test the first hypothesis were as follows:
  ...
  3. A T-test was used to examine whether or not these values were significant different at a level of significance of 5%.
...
The steps to test the second hypothesis were as follows:
  1. As the shot clock variable contained 5000 missing values, those were excluded from the analysis
...
The accuracies for each period are shown in the table below. 

Period  | Accuracy
------- | ---------
1       | `r print(df.acc$accuracy[1], digits = 2)`
2       | `r print(df.acc$accuracy[2], digits = 2)`
3       | `r print(df.acc$accuracy[3], digits = 2)`
4       | `r print(df.acc$accuracy[4], digits = 2)`
5       | `r print(df.acc$accuracy[5], digits = 2)`
6       | `r print(df.acc$accuracy[6], digits = 2)`
7       | `r print(df.acc$accuracy[7], digits = 2)`


The T-tests for period 1 and period 2 was significant, t(65301) = 2.42, p = 0.015, as well as the T-test for period 3 and 4, t(60757) = 4.24, p < 0.01. Further the T-test for period 2 and 3 was not significant, t(63843) = -1.53, p = 0.126, as well as the T-test for period 1 and 3, t()
...
<<<<<<< HEAD
top.scorers.list <- c("lebron james", "james harden", "russell westbrook", "stephen curry", "anthony davis", "blake griffin", "lamarcus aldridge", "cj miles", "kobe bryant", "marc gasol", "dwayne wade", "brandon knight", "andrew wiggins", "carmelo anthony", "nikola vucevic", "michael carter-williams", "eric bledsoe", "demarcus cousins", "kawhi leonard", "kyle lowry", "gordon hayward", "john wall") # players picked out
...
=======
top.scorers.list <- c("lebron james", "james harden", "russell westbrook", "stephen curry", "anthony davis", "blake griffin", "lamarcus aldridge") # players picked out
top.scorers <- shot.stats[shot.stats$player_name %in% top.scorers.list, c("player_name", "W", "LOCATION", "FINAL_MARGIN", "FGA", "FG.percentage")] # create a subset for top scorers
>>>>>>> 3d446d3ae38210f7e380e41539b94208ca820936
