...
=======
knitr::opts_chunk$set(echo = TRUE)
>>>>>>> 1dd00f3587e08e68ec99465d3cf26193d0b6d99e
```
...
The steps to calculate this measurement are as follows:  
    ...
    7. Discuss the findings of the data
...
###Introduction
  ...
  NULL Hypothesis: There is no significant difference between streaks of consequently scored shots and randomly simulated streaks for these players.  
  

###Methods
The steps to analyse this are are as follows:
...
####Plot Result
  When running the sample, the distributions of sample and real shooter are relatively similar. As a consequence, it can be concluded that Mo Williams does not have a hot hand.
...
set.seed(57)
shot_results <- c('made', 'missed')
simul_shooter <- sample(shot_results, size = sum(best_players_df[2,4], best_players_df[2,5], best_players_df[2,6]), replace = TRUE, prob = c(shooting_percentage[[2]], 1 - shooting_percentage[[2]]))
simul_shooter
