...
restdays<-cast(df1, DATE+ HOME_TEAM + AWAY_TEAM + WINNER ~ FGM)
```
...
This area of anlysis focuses on investigation of whether playing at 'home' gives the team advantage comparing to playing 'away'. 
...
for (i in (1:nrow(df.acc.players))){
  df.acc.players[i, "acc.per.sec"] <- sum(df.acc.players$'0'[df.acc.players$seconds.left == df.acc.players$seconds.left[i] & df.acc.players$player_id == df.acc.players$players_id[i]])} ### doesnt work as I would like it
...
grid.arrange(g1, g2, ncol = 2)
#Klay Thompson is an exception, had 13 streaks in a row! We cannot find a similar distribution in our sample, so we can assume that he is likely to have a hot hand. Fearther statistical analysis (logistic regression) should be done in order to reach a conclusion.
```
