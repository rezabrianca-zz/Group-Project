...
>>>>>>> fe3cfa4e9f195bbb483d19f7a4403fc4c37ed816
...
graph01 <- graph01 + labs(title = "Differences in FGA for games lost/won of the three special players") + xlab("Player name") + ylab("FGA")
graph02 <- ggplot(special2) + geom_boxplot(aes(x = player_name, y = FG.percentage, colour = W)) 
graph02 <- graph02 + labs(title = "Differences in FG% for games lost/won of the three special players") + xlab("Player name") + ylab("FG%")
...
###Plot Result
  The explanation from the plot above is summerised in the following table:  
  
** Table 13**: Relationship between the defender distance and accuracy 
...
###Correlation Test
  The next analysis calculates the correlation between both variables. If the analysis included all of the data points for every distance, the correlation result would be as follows:
...
**Table 14**: Relationship between the shooter distance ranges and accuracy

  Shooter Distance (in feet) | Accuracy (range in %)
  - | -
  0 - 2 | 62 - 68
  2 - 4 | 61 - 67
  4 - 6 | 44 - 58
