...
# find Feild Goal % of each indivudual player in that match

game[, "FG%"] <- NA 
for (i in 1:nrow(game)){
  game[i, "FG%"] <- round(((game[i, "1"]/game[i, "FGA"]) * 100), digits = 2)
}
...
dist_group <- c(5, 10, 15, 20, 25, Inf)
valid_distance[, "Range Group"] <- c(findInterval(valid_distance$distance, dist_group))
...
ggplot(data = valid_distance, aes(x = distance, y = shot_dist)) + geom_point()
